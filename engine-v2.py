...
PHYSICS_VERSION = "v2-headball-arcade"
...
class GameConfig:
    ...
    ball_max_speed: float = 920.0
    player_width: float = 58.0
    player_height: float = 72.0
    player_speed: float = 320.0
    player_jump_speed: float = 700.0
    jump_cooldown: float = 0.44
    kickoff_freeze: float = 0.90
    match_time: float = 60.0
    physics_fps: int = 60
    record_fps: int = 20
    body_ball_impulse_scale: float = 0.05  # kept for config compatibility
    floor_bounce: float = 0.54
    floor_friction: float = 0.965
    horizontal_drag_per_60fps: float = 0.997

    # Physics V2 additions.
    physics_substeps: int = 2
    player_acceleration: float = 2100.0
    player_deceleration: float = 2700.0
    player_air_acceleration: float = 900.0
    player_air_deceleration: float = 160.0
    player_gravity: float = 1850.0

    # Head Ball-style player collider: round head + compact torso.
    head_radius: float = 25.0
    head_center_y: float = 19.0
    body_inset_x: float = 7.0
    body_top_offset: float = 30.0

    goal_post_radius: float = 7.0
    ball_wall_bounce: float = 0.78
    ball_ceiling_bounce: float = 0.72
    ball_body_restitution: float = 0.34
    ball_head_restitution: float = 0.60
    ball_contact_impulse_cap: float = 470.0
    ball_sleep_speed: float = 14.0

    kick_reach: float = 115.0
    kick_low_x: float = 500.0
    kick_low_y: float = -130.0
    kick_high_x: float = 390.0
    kick_high_y: float = -450.0
    kick_clear_x: float = 610.0
    kick_clear_y: float = -270.0
    kick_low_cooldown: float = 0.56
    kick_high_cooldown: float = 0.66
    kick_clear_cooldown: float = 0.78
    kick_keep_ball_velocity: float = 0.26
    kick_player_velocity_transfer: float = 0.28
    move_deadzone: float = 9.0
...
def _apply_kicks(world: World, intents: list[Intent], config: GameConfig):
    ...
    # Simultaneous kicks are solved symmetrically. No blue/red processing bias.
    count = len(impulses)
    impulse_x = sum(ix for ix, _ in impulses) / count
    impulse_y = sum(iy for _, iy in impulses) / count
    ball = world.ball
...
def _resolve_players(world: World, config: GameConfig):
    ...
    # Horizontal separation is much more stable for this side-view arcade
    # game than the old order-sensitive push logic.
    a_center = a.x + config.player_width / 2
    b_center = b.x + config.player_width / 2
    direction = -1.0 if a_center <= b_center else 1.0
    correction = overlap_x / 2 + 0.01
    a.x += direction * correction
    b.x -= direction * correction

    closing_speed = (a.vx - b.vx) * direction
    if closing_speed < 0:
        shared_vx = (a.vx + b.vx) / 2
        a.vx = shared_vx
        b.vx = shared_vx
...
def _resolve_player_ball_contacts(world: World, config: GameConfig):
    ...
    # the combined correction/impulse once. This removes blue/red iteration
    # order from contested touches.
    for _ in range(2):
        ...
        for player in world.players:
            ...
            corrections.append((nx * (penetration + 0.01), ny * (penetration + 0.01)))
